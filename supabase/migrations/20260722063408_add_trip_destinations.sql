create table public.trip_destinations (
  ...
  destination_type text not null default 'city'
    check (destination_type in ('city', 'country')),
...
create unique index trip_destinations_active_key_unique
  on public.trip_destinations (trip_id, destination_key)
  where deleted_at is null;

create unique index trip_destinations_active_sort_order_unique
  on public.trip_destinations (trip_id, sort_order)
  where deleted_at is null;

create unique index trip_destinations_active_primary_unique
...
grant select, insert, update on table public.trip_destinations to authenticated;

create policy "Users can view destinations for their own trips"
...
create policy "Users can add destinations to their own trips"
  on public.trip_destinations
  for insert
  to authenticated
  with check (
...
create policy "Users can update destinations for their own trips"
  on public.trip_destinations
  for update
  to authenticated
  using (
...
select
  ...
  'legacy:' || md5(
    lower(
      concat_ws(
        '|',
        coalesce(nullif(trim(trips.destination_city), ''), nullif(trim(trips.destination_city_ko), '')),
        coalesce(nullif(trim(trips.destination_country), ''), nullif(trim(trips.destination_country_ko), ''))
      )
    )
  ),
  coalesce(nullif(trim(trips.destination_city), ''), nullif(trim(trips.destination_city_ko), '')),
  nullif(trim(trips.destination_city_ko), ''),
  nullif(trim(trips.destination_country), ''),
  nullif(trim(trips.destination_country_ko), ''),
  'city',
  0,
  true
from public.trips
where trips.deleted_at is null
  and coalesce(
    nullif(trim(trips.destination_city), ''),
    nullif(trim(trips.destination_city_ko), '')
  ) is not null
  and not exists (
    select 1
    from public.trip_destinations
    where trip_destinations.trip_id = trips.id
